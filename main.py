...
MAIN LOGIC@t_client.on(events.NewMessage(chats=CHANNELS))
async def handler(event):codes = re.findall(r'\b[A-Z0-9]{8}\b', event.raw_text)
for code in codes:print(f"Code found: {code}")
try:res = b_client.crypto_box_redeem(code)
print(f"SUCCESS: {res}")
except Exception as e:
print(f"FAILED {code}: {e}")
async def start():await t_client.start()
print("BOT IS ONLINE AND HUNTING")
await t_client.run_until_disconnected()
if __name__ == '__main__':threading.Thread(target=run_web, daemon=True).start()
asyncio.run(start())
